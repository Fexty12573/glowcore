...
namespace Tests.UnitTests.Menus
{
    public class NewGameServiceTests
    {
        ...
        {
            // Track call order via a simple sequence: gateway.Delete must run before context.SetNewGame
            // because the service deletes first; SceneTransition runs last so it doesn't tear down state we still need.
            m_gateway.ExistsResult = true;
            m_service.StartNewGame("Aurora");
            Assert.AreEqual(1, m_gateway.DeleteCallCount);
            Assert.AreEqual(GameLaunchMode.NewGame, m_launchContext.Mode);
