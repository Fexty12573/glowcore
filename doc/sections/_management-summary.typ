...
The numbers back this up. The game runs at around 120 FPS on WebGL during active play, well above the 60 FPS target we set ourselves. Worlds load in around 4 seconds for a fresh map, though large late-game saves can take up to 20 seconds, exceeding the original 3-second budget we set before the game grew this large, and save files come in at a few kilobytes despite a 5 MB ceiling. We ran two external user tests with players outside the team. Both testers reached the GlowCore upgrade loop on their own within a few minutes and confirmed that the core loop is fun to play. Their feedback on the parts that confused them or needed polish went straight into the next sprint.

On the engineering side the codebase stays consistent thanks to the automated checks, and the architecture proved its value when we added additional crafting stations late in the project without having to rewrite existing systems. The hours we tracked across the team came to 505, slightly above the 480 hour budget we planned at the start, which tells us that our storypoint estimates and sprint planning have been reasonably accurate.
