...
#nfr_table(
  ...
  result: "Manually tested by reloading the WebGL build 3 times (no Start button implemented yet). Average load time was approximately 4 seconds, exceeding the 3-second threshold. Note that WebGL builds are inherently slower than native builds due to browser overhead. Native Windows build performance has not yet been measured. Full automated benchmark across 10 consecutive loads is planned for M09.",
...
#nfr_table(
  ...
  result: "save file not yet implemented in the game",
...
#nfr_table(
  ...
  result: "Manually tested. The locked area border is visually communicated to the player. However, several restriction and interaction indicators are missing. There is no progress indicator when breaking nodes, no interaction prompt when hovering over workstations such as the crafting table, and no visual feedback for other restricted actions. Full visual feedback system is not yet implemented.",
...
#nfr_table(
  ...
  result: "Manually verified. The GlowCore upgrade UI clearly displays the current level, progress bar with percentage, required materials and amounts, a preview of the next level appearance, and the effect of the next upgrade. All information updates in real time as materials are fed. All acceptance criteria met.",
...
#nfr_table(
  ...
  result: "save system not yet implemented",
...
#nfr_table(
  ...
  result: "Automated test coverage has not been measured. The Unity Code Coverage package is not yet configured. While some unit tests exist, no coverage report has been generated and the 40% minimum target cannot be verified. Coverage measurement and additional tests are planned for Sprint 5.",
